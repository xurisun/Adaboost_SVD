...
public class MuitModelTraining_Top_N {
	...
	public double train(Set trainSet, Set testSet, int[][] userFeature,
		...
		ArrayList<Integer> preTestRmses = new ArrayList<>();
		...
		for (int pass = 1; pass <= numPasses; pass++) {
			...
			for (int i = 0; i < models.size(); i++) {
				...
				num.remove(i);
				num.add(i, ModelJudge.modelGetPrecisionAndRecall(m, trainSet,
						testSet, n));
				// if (testSetRmses.get(i) < preTestRmses.get(i)) {
				if (num.get(i) > preTestRmses.get(i)) {
					preTestRmses.remove(i);
					preTestRmses.add(i, num.get(i));
		...
		int b = ModelJudge.modelGetPrecisionAndRecall(bests, trainSet, testSet,
				10);
		/*
		 * for (int pass = 1; pass <= numPasses; pass++) { int[] prm =
		 * MyMethod.qrandperm(trainSet.getSize()); for (int i = 0; i <
		 * trainSet.getSize(); i++) { int user = trainSet.getUsers()[prm[i]];
		 * int item = trainSet.getItems()[prm[i]]; double rating =
		 * trainSet.getRatings()[prm[i]]; for (int j = 0; j < models1.size();
		 * j++) { // models.get(j).nclUpdate(user, item, rating, models);
		 * models1.get(j).update(user, item, rating); } } for (int i = 0; i <
		 * models1.size(); i++) { RecModel m = models1.get(i);
		 * trainSetRmses1.remove(i); trainSetRmses1.add(i,
		 * ModelJudge.modelGetRmse(m, trainSet)); testSetRmses1.remove(i);
		 * testSetRmses1.add(i, ModelJudge.modelGetRmse(m, testSet));
		 * 
		 * if (testSetRmses1.get(i) < preTestRmses1.get(i)) {
		 * preTestRmses1.remove(i); preTestRmses1.add(i, testSetRmses1.get(i));
		 * timesNotUpdates1.remove(i); timesNotUpdates1.add(i, 0); best1[i] =
		 * m.myclone(); } else { int timesNotUpdate = timesNotUpdates1.get(i);
		 * timesNotUpdates1.remove(i); timesNotUpdates1.add(i,
		 * ++timesNotUpdate); if (timesNotUpdates1.get(i) == 10) { String result
		 * = "Stop updating: model " + i + " rmse= " + preTestRmses1.get(i);
		 * results1.add(result); models1.remove(i); bests1.add(best1[i]);
		 * preTestRmses1.remove(i); timesNotUpdates1.remove(i);
		 * trainSetRmses1.remove(i); testSetRmses1.remove(i); break; } } } if
		 * (models1.size() == 0) break; }
